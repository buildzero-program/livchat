		...
		return (
			...
			>
				...
				>
					<div
						data-sidebar="sidebar"
						className="relative flex h-full w-full flex-col bg-sidebar overflow-hidden group-data-[variant=inset]:rounded-lg group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-sm"
					>
						{children}
						{/* Resize handle anchored inside the rounded, clipped container */}
						<SidebarRail />
					</div>
				</div>
			</div>
...
>(({ className, enableDrag = true, ...props }, ref) => {
	...
	return (
		<button
			...
			// aria values are approximate (rem->px @16px root)
			aria-valuemin={14 * 16}
			aria-valuemax={22 * 16}
			aria-valuenow={width.endsWith("rem") ? Math.round(parseFloat(width) * 16) : Math.round(parseFloat(width))}
			// make it focusable for keyboard resize
			tabIndex={0}
			// onClick={toggleSidebar}
			//* replace onClick with onMouseDown
			onMouseDown={handleMouseDown}
			onKeyDown={(e) => {
				const step = e.shiftKey ? 32 : 16; // px
				const toPx = (v: string) =>
					v.endsWith("rem")
						? Math.round(parseFloat(v) * 16)
						: Math.round(parseFloat(v));
				const toRem = (px: number) => `${(px / 16).toFixed(3)}rem`;
				const minPx = 14 * 16;
				const maxPx = 22 * 16;
				if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
					e.preventDefault();
					const currentPx = toPx(width);
					let next = currentPx + (e.key === "ArrowRight" ? step : -step);
					next = Math.min(Math.max(next, minPx), maxPx);
					setWidth(toRem(next));
				}
			}}
			title="Resize Sidebar"
			className={cn(
				// Full-height hit area, anchored inside the aside
				"absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear sm:flex",
				// Position at the real inner edge of the sidebar
				"group-data-[side=left]:right-0 group-data-[side=right]:left-0",
				// Resize cursors depending on side and state
				"in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
				"[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
				// Notch: small indicator near the edge (clipped by parent's overflow-hidden)
				"after:absolute after:top-1/2 after:-translate-y-1/2 after:h-12 after:w-[3px] after:rounded-full after:bg-transparent after:transition-colors",
				"group-data-[side=left]:after:right-[0px] group-data-[side=right]:after:left-[0px] hover:after:bg-sidebar-border focus-visible:after:bg-sidebar-border active:after:bg-sidebar-border group-data-[dragging=true]:after:bg-sidebar-border",
				className,
			)}
			{...props}
		/>
